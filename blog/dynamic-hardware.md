...
1. You set CPU and RAM too low.\
   Then the job fails with out-of-memory errors, or it becomes mysteriously slow because memory spills to disk. The second case is often worse than the first. A crash is visible, swap is quiet. A pipeline that should take minutes can become a pipeline that takes hours, and nothing in the user's code necessarily looks wrong.
...
When the node saturates, Burla removes workers. For CPU, saturation means the whole node is (almost) out of available CPU capacity, not that a single core is busy. For memory, this means RAM is close to spilling onto disk. When a worker is killed, the input it was processing goes back onto the queue, where another worker can pick it up later.
...
## "Dynamic Hardware" requires horizontal scaling

Adjusting worker count inside a machine can vertically scale available resources per worker. \
To make this actually effective the scheduler must also add and remove machines during runtime.
...
For the majority of data workloads, this is already true. Parsing documents, embedding text, transforming files, running inference, or computing independent outputs. This is less appropriate for code that mutates shared external state without safeguards.
...
But when work items are idempotent, this is an awesome trade.

## We can eliminate the catch

We're building a version of dynamic hardware that does not need to kill workers at all.

Today, Burla runs workers in containers. The roadmap is to run workers inside VMs, then move the entire running VM to another machine when a node gets crowded. The worker, its memory, and any other processes it started would continue running uninterrupted. The overloaded node would get more space, and the workload would no longer need to be idempotent.

This is called live migration, and it's already in production at scale: [Google Cloud uses live migration](https://docs.cloud.google.com/compute/docs/instances/live-migration-process) to move running VMs during host maintenance without interrupting the workload, rebooting the instance, or changing the VM's application state from the guest's point of view. The majority of live-migrations go completely unnoticed, and pause VM-state for less than a second.

Using live migration for dynamic hardware would remove the idempotency requirement from this kind of rescheduling. Instead of killing the smallest workers around a large one, Burla could move them to machines with more room. If no good destination exists, Burla already has mechanisms for adding nodes during runtime, and can move running workers there.

This is the strongest version of dynamic hardware: users do not specify CPU or RAM, Burla keeps every node close to \~90% CPU or RAM utilization, no work is killed or thrown out, and it quietly rearranges hardware underneath your workload while it is running.
